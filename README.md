...
При иннициализации плагина создается экземпляр класса Model, в него передается объект с настройками.  Модель хранит в себе бизнес-логику приложения. Экземпляр класса создается, после прохождения всех проверок и  сохраняет в себе следующие данные:
 ...
 - шаг (```_step```)
 - одно или два выбранных значения (```_value``` или ```_range```)
 - опция - в обратном порядке (```_reverse```)
 - пользовательские значения (```_customValues```)
 - объект непреобразованных опции (```_options```)

Модель предоставляет возможность работать с тремя типами данных (```dataFormat```): числовые значения (```numeric```), даты (```date```), пользовательский диапазон(```custom```). Однако, модель хранит в себе только числовые значения. В случае с датами - данные переводятся в колличество миллисекунд с 1 января 1970 года 00:00:00, шаг уножается на колличество миллисекунд в сутках. В случае c пользовательским диапазоном - минимальное значение равно 0, максимальное - длина диапазона минус один. Шаг всегда равен единице. Использовать опцию ```reverse``` не рекомендуется.
...
Свойство ```_options``` сохраняет последние непреобразованные полученные извне данные (все). Используется для изменения модели, повторного прохождения проверок.

Метод **translate** выводит: 
 ...
 - Если не указана единица измерения длины ```_length```, применяются 'px'.
 - Разность между максимальным и минимальным значениями модели должна делиться без остатка на шаг шкалы ```_scaleStep```. Шаг шкалы должен быть кратным шагу модели
 - Для масок используется код, где val - это значение, полученное методом модели **translate**(см. выше).

 #### **Представитель**. Класс Presenter
 При создании экземпляра класса Presenter, в него передается модель, вид и наюлюдатель. Представитель является связующим звеном между пользователем, моделью и видом. Любые изменения модели или вида происходят посредством представителя:
 - События мыши - перетаскивание бегунка, клик по полю слайдера.
 - Изменения извне с помощью метода **change**.

 Метод **change** принимает объект с новыми данными. При наличии новых данных модели, снова вызываются все проверки, новые данные должны соответствовать требованиям (см. выше). При необходимости производятся некоторые изменения вида, в зависимости от того, к каким изменениям привели новые, полученные извне опции.

 При изменении ```value``` или ```range``` модели наблюдатель оповещает об этом своих подписчиков.